...
interface DishCustomizationsSectionProps {
  formData: DishFormData;
  onUpdate: (updates: Partial<DishFormData>) => void;
  errors?: Record<string, string>;
}

export function DishCustomizationsSection({
  formData,
  onUpdate,
  errors = {},
}: DishCustomizationsSectionProps) {
  const { data: modifierGroups, isLoading: modifierGroupsLoading } =
    useModifierGroups(formData.chefUserId);
  ...
  const updateCustomizationGroup = (
    index: number,
    updates: Partial<DishFormData["customizationGroups"][0]>
  ) => {
  ...
  const updateModifier = (
    ...
    updates: Partial<DishFormData["customizationGroups"][0]["modifiers"][0]>
  ) => {
    const newGroups = [...formData.customizationGroups];
  ...
  const createModifierGroupMutation = useMutation({
    mutationFn: async (name: string) => {
      if (!formData.chefUserId) throw new Error("Chef user ID is required");
      const response = await modifierGroupsService.createModifierGroup({
        name: name.trim(),
        chefUser: formData.chefUserId,
      });
      return response.data;
    },
    onSuccess: (newGroup) => {
      queryClient.invalidateQueries({ queryKey: ["modifier-groups", formData.chefUserId] });
    ...
    <Card>
      <div className="space-y-[var(--p-space-500)]">
        <div className="flex flex-col gap-[var(--p-space-200)] sm:flex-row sm:items-center sm:justify-between">
          <HelpText>Add modifier groups to allow customers to customize their dish.</HelpText>
          <Button
            type="button"
            variant="secondary"
            size="sm"
              ...
              >
                ...
                <div className="px-[var(--p-space-400)] py-[var(--p-space-400)] space-y-[var(--p-space-400)]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-[var(--p-space-300)]">
                    ...
                    {/* Selection Type — segmented toggle */}
                    <div>
